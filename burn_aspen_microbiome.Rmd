...
# 16S -------------------------------------------------------------
data_16S <- read_csv2phyloseq(otu.file = "ASV_16S.csv", 
                              taxonomy.file = "Tax_16S.csv", 
                              metadata.file = "Metadata_16S.csv")
...
# ITS -----------------------------------------------------------------
data_ITS <- read_csv2phyloseq(otu.file = "ASV_ITS.csv", 
                              taxonomy.file = "Tax_ITS.csv", 
...
shapiro.test(resid(model)) # doesn't pass, but the resid vs. fitted looks good, plus it gets worse with transformation
anova.lme(model, type="sequential", adjustSigma = T) # significant interaction (p = 0.011)
marginal <- lsmeans(model, ~"sample_type:burn_severity")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# Diversity reduced by burning in bulk soil
# Among unburned plots soils had the highest richness, then leaf, then roots and stem together
marginal <- lsmeans(model, ~"sample_type")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# Rhizosphere above soil above leaf above roots and stem
...
anova.lme(model, type="sequential", adjustSigma = T) # significant interaction (p = 0.048)
marginal <- lsmeans(model, ~"sample_type:burn_severity")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# no significant differences for burning
marginal <- lsmeans(model, ~"sample_type")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# Rhizosphere above soil above leaf above roots and stem
...
plot(model) # looks good
shapiro.test(resid(model)) # doesn't pass, but the resid vs. fitted looks good
anova.lme(model, type="sequential", adjustSigma = T) # marginal significant interaction (p = 0.058)
marginal <- lsmeans(model, ~"sample_type:burn_severity")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# no significant differences for burning
marginal <- lsmeans(model, ~"sample_type")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# Rhizosphere above soil above everything else
...
invSimp_16S %>% filter(burn_severity == "Unburned") %>%
...
# Only a decrease in leaf richness
...
# If we analyze this way, all belowground niches have significant difs. 
# (mostly H vs. U -- except for bulk soil which was B vs. U)
...
rich_16S %>% filter(sample_type == "Leaf") %>% group_by(burn) %>% dplyr::summarise(mean = mean(y))
...
plot(model) # looks good
shapiro.test(resid(model)) # doesn't pass, but the resid vs. fitted looks good, plus it gets worse with transformation
anova.lme(model, type="sequential", adjustSigma = T) # significant interaction (p < 0.001)
marginal <- lsmeans(model, ~"sample_type:burn_severity")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# Diversity reduced by burning in bulk soil
# Among unburned plots soils had the highest richness, then leaf, then roots and stem together
...
plot(model) # looks good
shapiro.test(resid(model)) # doesn't pass, but the resid vs. fitted looks good
anova.lme(model, type="sequential", adjustSigma = T) # significant interaction (p = 0.010)
marginal <- lsmeans(model, ~"sample_type:burn_severity")
cld(marginal, alpha = 0.05, Letters = letters, adjust = "Tukey") 
# no significant differences for burning
marginal <- lsmeans(model, ~"sample_type")
...
rich %>%
  ggplot(aes(x = sample_type, y = y, fill = burn_severity)) +
    geom_boxplot() +
    ...
    geom_vline(xintercept = c(1.5, 2.5, 3.5, 4.5, 5.5), linetype = "dotted") -> p_chao
...
leg <- get_legend(p_chao + theme(legend.position = "right"))
prow <- plot_grid(p_chao + theme(legend.position = "none"), p_shan + theme(legend.position = "none"), p_invSimp + theme(legend.position = "none"),
                  ncol = 1, labels = "AUTO", align = "vh")
prow2 <- plot_grid(prow, leg, ncol = 2, rel_widths = c(1, 0.2))

png("~/Desktop/MCBurnRecovery/Figures/Fig_1.png", res = 300, height = 7.5, width = 6.5, units = "in")
prow2
dev.off()
```
...
perm <- how(complete = TRUE, 
            within   = Within(type = "none"),
            plots    = with(df_ITS, Plots(strata = plot, type = "free")))
...
fig.S1A <- ggplot(p$data, p$mapping) +
...
fig.S1B <- ggplot(p$data, p$mapping) +
    geom_point(aes(color = burn_severity, shape = sample_type), size = 4, alpha = 0.5) +
    #scale_color_manual(values = c("grey", "#BF2F38")) +
    scale_color_manual(values = c("grey", "orange", "#BF2F38")) +
    scale_x_continuous(name = paste0(paste0("PCoA 1 (", round(100*as.data.frame(ordu$values)[1,2], 1)), "%)")) +
...
leg <- get_legend(fig.S1A + theme(legend.position = "right"))
prow <- plot_grid(fig.S1A + theme(legend.position = "none"), fig.S1B + theme(legend.position = "none"), 
                  ncol = 1, labels = "AUTO", align = "vh")
Fig.S2 <- plot_grid(prow, leg, ncol = 2, rel_widths = c(1, 0.3))

tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S2.tiff", res = 100, height = 6, width = 6, units = "in")
Fig.S2
dev.off()
```
...
for(i in levels(as.factor(data_16S@sam_data$sample_type))){
  sub <- subset_samples(data_16S, sample_type %in% i) #subset by compartment
  samp <- data.frame(sub@sam_data) # create sample data matrix
  dist <- phyloseq::distance(sub, "bray") # create distance matrix
  set.seed(12290)
  ad <- adonis(dist ~ burn_severity, data = samp, permutations = 9999) # run permanova
...
# no difference for roots

# No differences between high and moderate burn
for(i in levels(as.factor(data_16S@sam_data$sample_type))){
  sub <- subset_samples(data_16S, sample_type %in% i) #subset by compartment
...
for(i in levels(as.factor(data_16S@sam_data$sample_type))){
  ...
  dist <- phyloseq::distance(sub, "bray") # create distance matrix
  set.seed(12290)
  ad <- adonis(dist ~ burn, data = samp, permutations = 9999) # run permanova
  x$name <- i # this and below populate the matrix named "x"
  x$`p-val` <- p.adjust(ad$aov.tab[1,6], method = "fdr", n = 6)
  x$`R^2` <- ad$aov.tab[1,5]
  tab <- rbind(tab, x) #concatenate "x" with "tab"
}
# but there is a difference between burn and unburn
...
for(i in levels(as.factor(data_ITS@sam_data$sample_type))){
  sub <- subset_samples(data_ITS, sample_type %in% i) #subset by compartment
  samp <- data.frame(sub@sam_data) # create sample data matrix
  dist <- phyloseq::distance(sub, "bray") # create distance matrix
  set.seed(12290)
  ad <- adonis(dist ~ burn_severity, data = samp, permutations = 9999) # run permanova
...
# no difference for roots, stem is marginal (p = 0.068)
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_2.tiff", res = 100, height = 4.5, width = 6.5, units = "in")
fig.2
dev.off()
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_3.tiff", res = 100, height = 4.5, width = 6.5, units = "in")
fig.3
dev.off()
```

# 16S Taxonomy (Fig S3 through S6)
```{r}
# Major Taxa -------------------------------------------
...
fig.S3 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = group)) +
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S3.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S3
dev.off()
...
fig.S4 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = Order)) +
    geom_bar(position = "stack", stat = "identity", col = "black") +
    facet_wrap(~sample_type, ncol = 3) +
    scale_fill_manual(values = c("#ff7f0e", "#0e334d","#1f77b4", "#abd2ed", "yellow",
                                 "#8c564b", "#d62728", "#9467bd", "#2ca02c", "#e377c2", 
...
tiff("~/Desktop/MCBurnRecovery/Figures/Figure_S4.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S4
dev.off()
...
fig.S5 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = Family)) +
    geom_bar(position = "stack", stat = "identity", col = "black") +
    facet_wrap(~sample_type, ncol = 3) +
    scale_fill_manual(values = c("#ff7f0e", "#0e334d","#1f77b4", "#abd2ed", "yellow",
                                 "#8c564b", "#d62728", "#9467bd", "#2ca02c", "#e377c2", 
...
tiff("~/Desktop/MCBurnRecovery/Figures/Figure_S5.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S5
dev.off()
...
tiff("~/Desktop/MCBurnRecovery/Figures/Figure_S6.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S6
dev.off()
```
...
fig.S7 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = group)) +
    geom_bar(position = "stack", stat = "identity", col = "black") +
    facet_wrap(~sample_type, ncol = 3) +
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S7.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S7
dev.off()
...
fig.S8 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = Family)) +
    geom_bar(position = "stack", stat = "identity", col = "black") +
    facet_wrap(~sample_type, ncol = 3) +
    scale_fill_manual(values = c("#ff7f0e","#1f77b4", "yellow",  "#2ca02c", "#d62728", 
                                 "#9467bd", "#8c564b", "#e377c2",  "#17becf", 
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S8.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S8
dev.off()
...
fig.S9 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = Genus)) +
    geom_bar(position = "stack", stat = "identity", col = "black") +
    facet_wrap(~sample_type, ncol = 3) +
    scale_fill_manual(values = c("#ff7f0e","#1f77b4", "yellow",  "#2ca02c", "#d62728", 
                                 "#9467bd", "#8c564b", "#e377c2",  "#17becf", 
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S9.tiff", res = 100, height = 7, width = 6.5, units = "in")
fig.S9
dev.off()
```
...
# Soils ----------------------------------------------------------------------
## Are soils different? ----------------------------------------------------
key <- read_excel("~/Desktop/MCBurnRecovery/Data/Burn_sample_key.xlsx")
soil_chem <- read_excel("~/Desktop/MCBurnRecovery/Data/Soil_chemistry.xlsx", skip = 8)
...
plot(a)
shapiro.test(resid(a)) # Pass
summary(a) # 0.016
TukeyHSD(a) # Difference between unburned and high (p = 0.012)

a <- aov(log(`pH 2`) ~ as.factor(`Burn Severity`), data = soil_chem %>% filter(sample_type == "Rhizosphere"))
plot(a)
shapiro.test(resid(a)) # Pass
summary(a) # 0.005
TukeyHSD(a) # High is higher than Unburned and Moderate

a <- aov(log(`NH4-N`+ 1) ~ as.factor(`Burn Severity`), data = soil_chem %>% filter(sample_type == "Bulk soil"))
plot(a)
shapiro.test(resid(a)) # Pass
summary(a) # 0.001
TukeyHSD(a) # Difference between unburned and high and unburn and moderate (both: p < 0.001)

a <- aov(log(`NO3-N` + 1) ~ as.factor(`Burn Severity`), data = soil_chem %>% filter(sample_type == "Bulk soil"))
plot(a)
shapiro.test(resid(a)) # Pass
summary(a) # 0.016
TukeyHSD(a) # Difference between unburned and high (p = 0.018)
...
## overall test
set.seed(1221990)
anova(rda_16S_rhizo, permutations = 9999) # p < 0.001
## Marginal or Type III effects
set.seed(1221990)
anova(rda_16S_rhizo, by="mar") # Everything is sig.
rda_16S_rhizo # 16.1%
...
## overall test
set.seed(1221990)
anova(rda_ITS_soil, permutations = 9999) # p < 0.001
## Marginal or Type III effects
set.seed(1221990)
anova(rda_ITS_soil, by="mar") # Everything but pH is sig.
rda_ITS_soil # 27.2%
...
## overall test
set.seed(1221990)
anova(rda_ITS_rhizo, permutations = 9999) # p < 0.001
## Marginal or Type III effects
set.seed(1221990)
anova(rda_ITS_rhizo, by="margin") # Everything is sig.
rda_ITS_rhizo # 17.0%
...
key <- read_excel("~/Desktop/MCBurnRecovery/Data/Burn_sample_key.xlsx")
plant_chem <- read_excel("~/Desktop/MCBurnRecovery/Data/Plant_chem_w_C.xlsx", skip = 8)
...
## overall test
set.seed(1221990)
anova(rda_16S_leaf, permutations = 9999) # p = 0.288
...
## overall test
set.seed(1221990)
anova(rda_16S_stem, permutations = 9999) # p = 0.553
rda_16S_stem
...
## overall test
set.seed(1221990)
anova(rda_16S_rhizome, permutations = 9999) # p = 0.501
rda_16S_rhizome
...
## overall test
set.seed(1221990)
anova(rda_16S_fine, permutations = 9999) # p = 0.745
...
## overall test
set.seed(1221990)
anova(rda_ITS_leaf, permutations = 9999) # p = 0.017
set.seed(1221990)
...
## overall test
set.seed(1221990)
anova(rda_ITS_stem, permutations = 9999) # p = 0.202
rda_ITS_stem
...
## overall test
set.seed(1221990)
anova(rda_ITS_rhizome, permutations = 9999) # p = 0.316
rda_ITS_rhizome
...
## overall test
set.seed(1221990)
anova(rda_ITS_fine, permutations = 9999) # p = 0.233
rda_ITS_fine

# Plots ----------------------------------------------------
plot.data <- merge(summary(rda_16S_soil)$sites, data.frame(sample_data(data_16S %>% subset_samples(sample_type == "Bulk soil"))), by = "row.names") 
ggplot() +
...
png("~/Desktop/MCBurnRecovery/Figures/Fig_4.png", res = 100, height = 4.5, width = 6.5, units = "in")
prow2
dev.off()
...
ggplot() +
    ...
    geom_text_repel(data = as.data.frame(summary(rda_ITS_leaf)$biplot) %>% 
                    ...
                    aes(x = dbRDA1, y = dbRDA2, label = rowname), 
                    size = 5) -> p_ITS_leaf
png("~/Desktop/MCBurnRecovery/Figures/Fig_S10.png", res = 100, height = 4.5, width = 6.5, units = "in")
p_ITS_leaf
dev.off()
...
plant_chem %>%
  ...
  spread(key = `Burn Severity`, value = "value") -> table.S4
```
...
```{r}
# 16S ---------------------------------------------
ST_16S <- read.delim("~/Desktop/MCBurnRecovery/Output/051820_16S_source_contributions_matrix.txt")
...
summary(x) # significant interaction (p = 0.0496)
TukeyHSD(x) # high coarse, greater than everything

x <- aov(Proportion ~ burn_severity*Source, data = ST_16S_g %>% filter(sample_type == "Leaf"))
shapiro.test(resid(x))
summary(x) # nothing sig.
...
# ITS --------------------------------------------------------------------------------------
ST_ITS <- read.delim("~/Desktop/MCBurnRecovery/Output/051520_ITS_source_contributions_matrix.txt")
...
} # Only Rhizome/Stem sig. p = 0.003

x <- aov(Proportion ~ burn_severity*Source, data = ST_ITS_g %>% filter(sample_type == "Stem" & Source != "<NA>"))
shapiro.test(resid(x))
summary(x) # significant interaction (p < 0.001)
TukeyHSD(x) # unburn rhizo, greater than everything

x <- aov(Proportion ~ burn_severity*Source, data = ST_ITS_g %>% filter(sample_type == "Leaf"))
shapiro.test(resid(x))
summary(x) # significant interaction (p < 0.001)
TukeyHSD(x) # unburn rhizo, greater than but mod rhizo
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_4.tiff", res = 100, height = 4.5, width = 6.5, units = "in")
p_ST
dev.off()
```

# Fungal guilds (Fig 5, S10 and S11)
```{r}
# Set up ------------------------------------------------
fg <- parse_funguild()
...
x <- aov(sum ~ burn_severity, EM_sum %>% filter(sample_type == "Fine root"))
plot(x)
shapiro.test(resid(x))
summary(x)
x <- aov(sum ~ burn_severity, EM_sum %>% filter(sample_type == "Rhizosphere"))
plot(x)
shapiro.test(resid(x))
summary(x)
x <- aov(sum ~ burn_severity, EM_sum %>% filter(sample_type == "Bulk soil"))
plot(x)
shapiro.test(resid(x))
summary(x)

## Richness ------------------------------------
data_ITS_EM_rich <- data_ITS_EM
otu_table(data_ITS_EM_rich)[otu_table(data_ITS_EM_rich) > 0.0000000001] <- 1
otu_table(data_ITS_EM_rich)[otu_table(data_ITS_EM_rich) < 0.0000000001] <- 0
EM_rich <- estimate_richness(data_ITS_EM_rich)
EM_rich <- merge(data_ITS_EM@sam_data, 
                 EM_rich %>% rownames_to_column(var = "Sample") %>% mutate(Sample = gsub("[.]", "-", Sample)), 
                 by.x = 0, by.y = "Sample")

model <- lme(Observed ~ sample_type*burn_severity, random=~1|plot, data = EM_rich, method="REML")
plot(model) # looks good
shapiro.test(resid(model)) # doesn't pass, but the resid vs. fitted looks good, plus it gets worse with transformation
anova.lme(model, type="sequential", adjustSigma = T) # significant interaction (p = 0.011)

x <- aov(Observed ~ burn_severity, EM_rich %>% filter(sample_type == "Fine root"))
plot(x)
shapiro.test(resid(x))
summary(x)
x <- aov(Observed ~ burn_severity, EM_rich %>% filter(sample_type == "Rhizosphere"))
plot(x)
shapiro.test(resid(x))
summary(x)
x <- aov(Observed ~ burn_severity, EM_rich %>% filter(sample_type == "Bulk soil"))
plot(x)
shapiro.test(resid(x))
summary(x)
...
set.seed(01221990)
mod <- adonis(dist_16S ~ sample_type*burn_severity, data = df_16S, permutations = 9999)
# sample type: p = 0.040, burn_severity: p = 0.003
...
x <- aov(sum ~ burn_severity, AM_sum %>% filter(sample_type == "Fine root"))
plot(x)
shapiro.test(resid(x))
summary(x)
x <- aov(sum ~ burn_severity, AM_sum %>% filter(sample_type == "Rhizosphere"))
plot(x)
shapiro.test(resid(x))
summary(x) #F2,24 = 4.512, p = 0.022
x <- aov(sum ~ burn_severity, AM_sum %>% filter(sample_type == "Bulk soil"))
plot(x)
shapiro.test(resid(x))
summary(x) #F2,24 = 9.506, p < 0.001

## Richness ------------------------------------
data_ITS_AM_rich <- data_ITS_AM
otu_table(data_ITS_AM_rich)[otu_table(data_ITS_AM_rich) > 0.0000000001] <- 1
otu_table(data_ITS_AM_rich)[otu_table(data_ITS_AM_rich) < 0.0000000001] <- 0
AM_rich <- estimate_richness(data_ITS_AM_rich)
AM_rich <- merge(data_ITS_AM@sam_data, 
                 AM_rich %>% rownames_to_column(var = "Sample") %>% mutate(Sample = gsub("[.]", "-", Sample)), 
                 by.x = 0, by.y = "Sample")

x <- aov(Observed ~ burn_severity, AM_rich %>% filter(sample_type == "Fine root"))
plot(x)
shapiro.test(resid(x))
summary(x)
x <- aov(Observed ~ burn_severity, AM_rich %>% filter(sample_type == "Rhizosphere"))
plot(x)
shapiro.test(resid(x))
summary(x) #F2,24 = 3.979, p = 0.032
x <- aov(Observed ~ burn_severity, AM_rich %>% filter(sample_type == "Bulk soil"))
plot(x)
shapiro.test(resid(x))
summary(x) #F2,24 = 11.840, p < 0.001

## Composition ---------------------------------------------
data_ITS_AM_rel <- transform_sample_counts(data_ITS_AM, function(x) x/sum(x))
data_ITS_AM_rel <- subset_samples(data_ITS_AM_rel, 
                                  sample_type %in% c("Fine root", "Rhizosphere", "Bulk soil") & 
                                    sample.id != "U06-fine-ITS")
data_ITS_AM_rel <- prune_samples(sample_sums(data_ITS_AM_rel)>=0.5, data_ITS_AM_rel)

df_16S <- data.frame(sample_data(data_ITS_AM_rel))
set.seed(12290)
OTU_16S <- t(as(data_ITS_AM_rel@otu_table, "matrix"))
OTU_16S <- as.data.frame(OTU_16S)
dist_16S <- vegdist(OTU_16S, "bray", na.rm = T)
...
set.seed(01221990)
mod <- adonis(dist_16S ~ sample_type*burn_severity, data = df_16S, permutations = 9999)
# No difference in overall composition
...
PATH_melt %>% 
  filter(sample_type == "Leaf" & ta1 %in% c("Alternaria", "Cladosporium")) %>% 
  #na.omit() %>%
  group_by(burn_severity, ta1) %>% 
  summarise(mean = mean(Abundance))

# Richness ----------------------------------------------
data_ITS_PATH_rich <- data_ITS_PATH
otu_table(data_ITS_PATH_rich)[otu_table(data_ITS_PATH_rich) > 0.0000000001] <- 1
otu_table(data_ITS_PATH_rich)[otu_table(data_ITS_PATH_rich) < 0.0000000001] <- 0
PATH_rich <- estimate_richness(data_ITS_PATH_rich)
PATH_rich <- merge(data_ITS_PATH@sam_data, 
                   PATH_rich %>% rownames_to_column(var = "Sample") %>% mutate(Sample = gsub("[.]", "-", Sample)), 
                   by.x = 0, by.y = "Sample")

model <- lme(Observed ~ sample_type*burn_severity, random=~1|plot, data = PATH_rich, method="REML")
plot(model) # looks good
shapiro.test(resid(model)) # doesn't pass, but the resid vs. fitted looks good, plus it gets worse with transformation
anova.lme(model, type="sequential", adjustSigma = T) # significant interaction (p = 0.011)
marginal <- lsmeans(model, ~"sample_type:burn_severity")
...
for(i in levels(as.factor(PATH_rich$sample_type))){
  x <- aov(Observed ~ burn_severity, data = PATH_rich %>% filter(sample_type %in% i))
  print(i)
  print(p.adjust(summary(x)[[1]][[1,"Pr(>F)"]], "fdr", n = 6))
  print(summary(x)[[1]][[1,"F value"]])
  print(TukeyHSD(x))
...
mycorrhizal_richness <- merge(EM_rich %>% dplyr::select(sample_type, plot, burn_severity, Observed), 
                              AM_rich %>% dplyr::select(sample_type, plot, burn_severity, Observed), 
                              by = c("sample_type", "plot", "burn_severity"))

mycorrhizal_richness %>%
  gather(key = "type", value = "rich", -sample_type, -plot, -burn_severity) %>%
  mutate(type = dplyr::recode(type, Observed.x = "EM", Observed.y = "AM")) %>%
  filter(sample_type %in% c("Fine root", "Rhizosphere", "Bulk soil")) %>%
  group_by(sample_type, burn_severity, type) %>%
  ...
  theme(legend.position = "none", axis.text.x = element_text(size = 9)) +
  panel_border() +
  facet_grid(cols = vars(sample_type), rows = vars(type), scales = "free") -> p1

Fig.S11 <- plot_grid(p, p1, labels = "AUTO", align = "vh", ncol = 1)

tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S11.tiff", res = 100, height = 6, width = 6.5, units = "in")
Fig.S11
dev.off()

## EM ordinations ------------------------------------------------------
ordu <- ordinate(data_ITS_EM_rel, "PCoA", "bray", weighted = T)
p_ord <- plot_ordination(data_ITS_EM_rel, ordu)
p2 <- ggplot(p_ord$data, p_ord$mapping) +
    geom_point(aes(color = burn_severity, shape = sample_type), size = 4, alpha = 0.5) +
    #scale_color_manual(values = c("grey", "#BF2F38")) +
    scale_color_manual(values = c("grey", "orange", "#BF2F38")) +
    scale_x_continuous(name = paste0(paste0("PCoA 1 (", round(100*as.data.frame(ordu$values)[1,2], 1)), "%)")) +
...
myplots <- list()
for(i in levels(as.factor(data_ITS_EM_rel@sam_data$sample_type))){
  sub <- subset_samples(data_ITS_EM_rel, sample_type %in% i)
  ordu <- ordinate(sub, "PCoA", "bray", weighted = T)
  p <- plot_ordination(sub, ordu)
  p2 <- ggplot(p$data, p$mapping) +
    geom_point(aes(fill = burn_severity), size = 3, alpha = 0.5, pch = 21) +
...
fig.S10 <- ggplot(sum2, aes(x = burn_severity, y = means, fill = ta1)) +
    geom_bar(position = "stack", stat = "identity", col = "black") +
    facet_wrap(~sample_type, ncol = 3) +
    scale_fill_manual(values = c("#ff7f0e","#1f77b4", "yellow",  "#2ca02c", "#d62728", 
                                 "#9467bd", "#8c564b", "#e377c2",  "#17becf", 
...
tiff("~/Desktop/MCBurnRecovery/Figures/Fig_S10.tiff", res = 100, height = 6, width = 6.5, units = "in")
fig.S10
dev.off()
...
PATH_sum %>%
  ...
  facet_wrap(~sample_type) -> fig.5

tiff("~/Desktop/MCBurnRecovery/Figures/Fig_5.tiff", res = 100, height = 4, width = 6.5, units = "in")
fig.5
dev.off()
```
